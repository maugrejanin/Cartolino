...
export class TimeControllFake implements ITimeControll {
    ...
    getParciaisDosJogadoresDoTime(time: Time) {
        return new Promise((resolve, reject) => {
            ...
            this.calcularParcialTime(time).then(time => {
                resolve({ time: time, mercadoStatus: 1 })
            });
        });
    }
