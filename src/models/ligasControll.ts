...
import { ITimeControll } from "./timeControll";
    ...
    loadTimesInfo(times) {
        return new Promise((resolve, reject) => {
            ...
            for (let time of times) {
                this.timeControll.loadTime(time).then(timeInfo => {
                    timesOk.push(timeInfo);
                });
            }
            resolve(timesOk);
        });
